                      ...
                      <div key={uni.id} className={`border p-6 rounded-3xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-xl ${isDark ? 'bg-gradient-to-r from-gray-900 to-gray-950 border-gray-800' : 'bg-white border-gray-200'}`}>
                        <div className="space-y-2">
                          <div className="flex flex-wrap items-center gap-3">
                            <h4 onClick={() => setAdminSelectedUni(uni)} className="text-lg font-bold text-blue-400 cursor-pointer hover:underline transition">
                              {uni.name} 🔍
                            </h4>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <button onClick={() => handleAdminAction(uni.id, 'Approved')} className="bg-emerald-600 hover:bg-emerald-500 text-white px-3.5 py-2 rounded-xl text-xs font-semibold shadow">
                            Approve
                          </button>
                          <button onClick={() => handleAdminAction(uni.id, 'Rejected')} className="bg-amber-600 hover:bg-amber-500 text-white px-3.5 py-2 rounded-xl text-xs font-semibold shadow">
                            Reject
                          </button>
                          <button onClick={() => handleDeleteUniversity(uni.id)} className="bg-rose-600 hover:bg-rose-500 text-white px-3.5 py-2 rounded-xl text-xs font-semibold shadow">
                            Delete
                          </button>
                        </div>
                      </div>
              ...
              <div className={`border p-6 md:p-8 rounded-3xl shadow-xl max-w-3xl mx-auto space-y-6 ${isDark ? 'bg-gradient-to-b from-gray-900 to-gray-950 border-gray-800' : 'bg-white border-gray-200'}`}>
                {!isOrgLoggedIn ? (
                  <div className="space-y-6">
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); if(orgEmail && orgPassword){ setIsOrgLoggedIn(true); setOrgName(orgEmail.split('@')[0].toUpperCase()); alert("Organization Logged In Successfully!"); } }} className="space-y-4 max-w-md mx-auto">
                      <div>
                  ...
                  <div className="space-y-6">
                    <div className="flex justify-between items-center bg-blue-950/40 p-4 rounded-2xl border border-blue-800/40">
                      ...
                      <button onClick={() => setIsOrgLoggedIn(false)} className="bg-red-600 hover:bg-red-500 text-white px-3.5 py-1.5 rounded-xl text-xs font-semibold">
                        Org Logout
                      </button>
                    </div>
        ...
        {showRegModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
            <div className={`border p-6 md:p-8 rounded-3xl max-w-lg w-full shadow-2xl space-y-6 relative max-h-[90vh] overflow-y-auto ${isDark ? 'bg-gray-900 border-gray-800 text-white' : 'bg-white border-gray-200 text-gray-900'}`}>
              <button onClick={() => setShowRegModal(false)} className="absolute top-6 right-6 text-gray-400 hover:text-white text-lg font-bold">
                ✕
              </button>
